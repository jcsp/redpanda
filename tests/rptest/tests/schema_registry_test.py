...
class SchemaRegistryTest(RedpandaTest):
    ...
    def _request(self, verb, path, **kwargs):
        ...
        :param expect: Optional list of error response codes to not retry on
        :return:
        """
        ...
        def accept_response(resp):
            return 200 <= resp.status_code < 300 or resp.status_code in expect or resp.status_code in acceptable_errors
        ...
        if accept_response(r):
            ...
            self.logger.info(f"Retrying for error {r.status_code} on {verb} {path} ({r.text})")
            time.sleep(10)
            r = requests.request(verb, uri, **kwargs)
            if accept_response(r):
                self.logger.info(f"OK after retry {r.status_code} on {verb} {path} ({r.text})")
            else:
                self.logger.info(f"Error after retry {r.status_code} on {verb} {path} ({r.text})")
    ...
    def _post_subjects_subject_versions(self,
        ...
        return self._request("POST", f"subjects/{subject}/versions",
                             headers=headers,
                             data=data)
    ...
    def _get_subjects_subject_versions(self,
                                       subject,
                                       deleted=False,
                                       headers=HTTP_GET_HEADERS, **kwargs):
        return self._request("GET",
    ...
    def test_delete_subject(self):
        ...
        result_raw = self._delete_subject(subject=f"{topic}-key",
                                          permanent=True, expect={404})
        self.logger.debug(result_raw)
        assert result_raw.status_code == requests.codes.not_found

        self.logger.debug("Soft delete subject")
        result_raw = run_with_timeout(lambda: self._delete_subject(subject=f"{topic}-key"), 10)
        self.logger.debug(result_raw)
        assert result_raw.status_code == requests.codes.ok

        self.logger.debug("Get versions")
        result_raw = self._get_subjects_subject_versions(
            subject=f"{topic}-key", expect={404})
        self.logger.debug(result_raw)
        assert result_raw.status_code == requests.codes.not_found

...
This command erases a named property from the local node's internal 
cache, so that on next startup redpanda will treat the setting as if it 
			...
			for key, node := range inputDoc {
				if key == "redpanda" {
					...
					for i := 0; i < len(node.Content); i += 2 {
						...
						key := keyNode.Value
						_, is_cluster_config := schema[key]
						if is_cluster_config {
							cleanedProperties = append(cleanedProperties, key)
						} else {
							cleanedContent = append(cleanedContent, keyNode)
							cleanedContent = append(cleanedContent, valNode)
						}
					}

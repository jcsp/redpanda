...
topic_updates_dispatcher::apply_update(model::record_batch b) {
    ...
    return deserialize(std::move(b), commands).then([this, base_offset](auto cmd) {
        return ss::visit(
          std::move(cmd),
          [this, base_offset](delete_topic_cmd del_cmd) {
              ...
              topic_lifecycle_transition transition{
                .topic
                = nt_revision{.nt = del_cmd.key, .initial_revision_id = model::initial_revision_id{0}},
                .mode = topic_lifecycle_transition_mode::oneshot_delete,
              };

              return do_topic_delete(transition, base_offset);
          },
          [this, base_offset](topic_lifecycle_transition_cmd soft_del_cmd) {
              return do_topic_delete(soft_del_cmd.value, base_offset);
          },
              ...
              return dispatch_updates_to_cores(create_cmd, base_offset)
                .then([this, create_cmd](std::error_code ec) {
                    if (ec == errc::success) {
                        ...
                        for (const auto& p_as : create_cmd.value.assignments) {
                            _partition_balancer_state.local().handle_ntp_update(
                              tp_ns.ns, tp_ns.tp, p_as.id, {}, p_as.replicas);
                        }
                    }
          ...
          [this, base_offset](revert_cancel_partition_move_cmd cmd) {
              ...
              // an example above)
              auto previous_replicas
                = _topic_table.local().get_previous_replica_set(cmd.value.ntp);
              auto target_replicas

...
topic_table::apply(topic_lifecycle_transition soft_del, model::offset offset) {
    ...
    if (soft_del.mode == topic_lifecycle_transition_mode::pending_gc) {
        ...
        auto tombstone = nt_lifecycle_marker{
          .config = tp->second.get_configuration(), .partition_markers = {}};

        for (const auto& p : tp->second.get_assignments()) {
            tombstone.partition_markers.push_back(ntp_lifecycle_marker{
              .id = p.id,
              .initial_revision_id = model::initial_revision_id{
        ...
        if (_lifecycle_markers.contains(soft_del.topic)) {
            ...
            // TOOD: should we assert here?  Or ignore the error?  In principle
            // it is harmless, but indicates a bug.
            return ss::make_ready_future<std::error_code>(
              errc::topic_not_exists);
        }
    }

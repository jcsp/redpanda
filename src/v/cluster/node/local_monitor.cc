...
ss::future<> local_monitor::update_disks(local_state& state) {
    if (_path_for_test.empty()) {
        ...
        state.data_disk = ss::make_lw_shared<storage::disk>(
          statvfs_to_disk(data_svfs));
        if (cache_svfs.f_fsid != data_svfs.f_fsid) {
            state.cache_disk = ss::make_lw_shared<storage::disk>(
              statvfs_to_disk(cache_svfs));
        } else {
            state.cache_disk = state.data_disk;
        }
    } else {
        // Test mode
        auto svfs = co_await get_statvfs(_path_for_test);
        state.data_disk = ss::make_lw_shared<storage::disk>(
          statvfs_to_disk(svfs));
        state.cache_disk = state.data_disk;
    }
}
...
void local_monitor::update_alert_state(local_state& state) {
    vassert(state.data_disk, "Data disk not set");
    vassert(state.cache_disk, "Cache disk not set");
    update_alert(*(state.data_disk));
    maybe_log_space_error(*(state.data_disk));
    if (!state.shared_disk()) {
        update_alert(*(state.cache_disk));
        maybe_log_space_error(*(state.cache_disk));
    }
}

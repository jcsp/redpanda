...
bool operator==(const local_state& a, const local_state& b) {
    return a.redpanda_version == b.redpanda_version
           && a.logical_version == b.logical_version && a.uptime == b.uptime
           && *(a.data_disk) == *(b.data_disk)
           && *(a.cache_disk) == *(b.cache_disk);
}
...
void local_state::serde_read(iobuf_parser& in, const serde::header& h) {
    ...
    // Redpanda <= 22.3 did not explicitly track data+cache drives, just sent
...
storage::disk_space_alert local_state::get_disk_alert() const {
    return storage::max_severity(data_disk->alert, cache_disk->alert);
}
...
void local_state::set_disk(storage::disk d) {
    data_disk = ss::make_lw_shared<storage::disk>(d);
    cache_disk = data_disk;
}

void local_state::set_disks(std::vector<storage::disk> v) {
    if (v.size() == 0) {
        // This is invalid input, but we can cope gracefully
        data_disk = ss::make_lw_shared<storage::disk>();
        cache_disk = data_disk;
    } else {
        data_disk = ss::make_lw_shared<storage::disk>(v[0]);
        if (v.size() > 1) {
            cache_disk = ss::make_lw_shared<storage::disk>(v[1]);
        } else {
            cache_disk = data_disk;
        }
    }

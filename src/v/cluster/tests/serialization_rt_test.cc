...
void adl_roundtrip_test(const T original) {
    ...
    auto from_adl = reflection::from_iobuf<T>(adl_out.copy());

    if (original != from_adl) {
        std::cerr << "Failed ADL roundtrip on " << typeid(T).name()
                  << std::endl;
        std::cerr << "Dump " << adl_out.size_bytes()
                  << " bytes: " << adl_out.hexdump(1024) << std::endl;
    }
...
cluster::node::local_state random_local_state() {
    auto data_disk = ss::make_lw_shared<storage::disk>(storage::disk{
    ...
    ss::lw_shared_ptr<storage::disk> cache_disk = data_disk;
    if (random_generators::get_int(0, 1) == 0) {
        cache_disk = ss::make_lw_shared<storage::disk>(storage::disk{
          .path = random_generators::gen_alphanum_string(
            random_generators::get_int(20)),
          .free = random_generators::get_int<uint64_t>(),
          .total = random_generators::get_int<uint64_t>(),
...
SEASTAR_THREAD_TEST_CASE(serde_reflection_roundtrip) {
    ...
    }
    {
        ...
        // to keep the default value because thsi field isn't serialized in adl.
        data.data_disk->alert = storage::disk_space_alert::degraded;
        serde_roundtrip_test(data);
    }
    {
    ...
    }
    {
        ...
        // test ADL roundtrip.
        data.local_state.cache_disk = data.local_state.data_disk;

...
storage_resources::storage_resources(
  ...
  , _consensus_stm_dirty_bytes(_target_replay_bytes() / ss::smp::count)
  , _inflight_recovery(_max_concurrent_replay() / ss::smp::count)
  , _inflight_close_flush(_max_concurrent_replay() / ss::smp::count) {
    ...
    _target_replay_bytes.watch([this]() {
        ...
        _consensus_stm_dirty_bytes.set_capacity(v);
...
storage_resources::consensus_stm_take_bytes(size_t bytes) {
    vlog(
      stlog.trace,
      "consensus_stm_take_bytes {} (current {})",
      bytes,
      _consensus_stm_dirty_bytes.current());

    return _consensus_stm_dirty_bytes.take(bytes);
}

 ...
 */
class adjustable_allowance : public ss::semaphore {
public:
    adjustable_allowance(uint64_t capacity)
      : ss::semaphore(capacity)
      , _capacity(capacity) {}

    /**
     * This must be called periodically or before taking units,
     * to account for decreases in semaphore size.
     */
    void maybe_adjust() noexcept {
        if (likely(_dirty == 0)) {
            return;
        }

        auto reclaim = std::min(current(), _dirty);
        _dirty -= reclaim;
        consume(reclaim);
    }
    ...
    void set_capacity(uint64_t capacity) noexcept {
        ...
        if (capacity > _capacity) {
            signal(capacity - _capacity);
        } else {
            _dirty += _capacity - capacity;
        }
    ...
    take_result take(size_t units) {
        ...
        take_result result = {
          .units = ss::consume_units(*this, units),
          .checkpoint_hint = current() <= 0};
...
private:
    ...
    // Units that were handed out and should now be destroyed
    // due to a decrease in capacity.
    uint64_t _dirty{0};
};
...
private:
    ...
    adjustable_allowance _consensus_stm_dirty_bytes{0};

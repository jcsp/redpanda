...
public:
    ...
    template<typename F>
    ss::future<> sync(F f) {
        auto sem_units = co_await ss::smp::submit_to(
          ss::shard_id{0},
          _smp_opts,
          std::move(
            [this]()
              -> ss::future<ss::foreign_ptr<std::unique_ptr<ss::semaphore_units<
                ss::semaphore_default_exception_factory,
                ss::timer<>::clock>>>> {
                co_return ss::make_foreign(
                  std::make_unique<ss::semaphore_units<
                    ss::semaphore_default_exception_factory,
                    ss::timer<>::clock>>(co_await ss::get_units(_sync_sem, 1)));
            }));

        co_await f();
    }

    ss::future<model::offset> begin_write();

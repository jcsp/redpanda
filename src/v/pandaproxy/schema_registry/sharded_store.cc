...
sharded_store::get_subject_written_at(const subject& sub) {
    auto history = co_await _store.invoke_on(
      shard_for(sub), &store::get_subject_written_at, sub);
...
ss::future<bool> sharded_store::delete_subject_version(
  const subject& sub, schema_version version) {
    auto deleted = co_await _store.invoke_on(
      shard_for(sub), &store::delete_subject_version, sub, version);
...
ss::future<model::offset> sharded_store::begin_write() {
    auto get_next = [this]() -> ss::future<model::offset> {
        co_await _seq_sem.wait(1);
        co_return _loaded_offset + model::offset{1};
    };
    auto where = co_await ss::smp::submit_to(
      ss::shard_id{0}, _smp_opts, std::move(get_next));

    vlog(plog.debug, "sharded_store: begin_write: {}", where);
